...
void Controller::process_input(std::vector<std::string>& args)
{
	...
	if (args[0].compare("add") == 0 && args.size() == 6)
	{
		try
		{
			if(args[0].empty() || args[1].empty() || args[2].empty() || args[3].empty() || args[4].empty() || args[5].empty())
				throw ControllerException("[ERROR] No arguments can be empty when adding");
	...
	else if (args[0].compare("update") == 0 && args.size() == 6)
	{
		...
		{
			if (args[0].empty() || args[1].empty() || args[2].empty() || args[3].empty() || args[4].empty() || args[5].empty())
				throw ControllerException("[ERROR] No arguments can be empty when updating");
